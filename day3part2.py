...
# The grid is represented as a mapping of 2D coordinates => contents of 
# the cell, which is a tuple of (wire_id, steps).
grid = {}

# Central port in the middle of the grid.
cx = cy = 0 
grid[(cx, cy)] = (0, 0)
...
# Remember the positions of crossing(s) found
crossings = set()
         

def tracewire(wire_id, path):
    x, y = cx, cy    
    delay = 1
    for part in path.split(","):
        ...
        while steps:
            x, y = x + dx, y + dy
            if (x, y) in grid:                
                if grid[(x, y)][0] != wire_id: # Crossing an other wire.
                    otherdelay = grid[(x, y)][1] 
                    grid[(x, y)] = (wire_id, delay + otherdelay)
                    crossings.add((x, y))
            else:
                grid[(x, y)] = (wire_id, delay)
            steps -= 1
            delay += 1


def printgrid():
    xcoords = [coord[0] for coord in grid.keys()]
    ycoords = [coord[1] for coord in grid.keys()]
...
def fewest_steps(crosses):
    min_found = None
    for cross in crosses:
        d = grid[cross][1]
        if min_found is None or d < min_found:
            min_found = d
    return min_found
